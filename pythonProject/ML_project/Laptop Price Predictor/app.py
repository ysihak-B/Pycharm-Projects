...
file1 = open('pipedt.pkl', 'rb')
dt = pickle.load(file1)
file1 = open('pipelr.pkl', 'rb')
...
screen_size = st.number_input('Screen Size', min_value=1.00)
...
if st.button('Predict Price'):
    ...
    prediction = int(np.exp(dt.predict(query)[0]))

    # prediction using linear regression
    # prediction = int(np.exp(lr.predict(query)[0]))

    st.title("Predicted price for this laptop could be between " +
             str(prediction - 1000) + "Birr" + " to " + str(prediction + 1000) + "Birr")
